...
export async function initDatabase() {
  ...
  if (!db) {
    console.warn("[Database] Cannot init: database not available");
    return;
  }
  try {
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS \`settlements\` (
      \`id\` int AUTO_INCREMENT PRIMARY KEY,
      \`orderDate\` bigint,
      \`orderNo\` varchar(64),
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS \`transfer_records\` (
      \`id\` int AUTO_INCREMENT PRIMARY KEY,
      \`imageData\` longtext,
      \`note\` text,
      \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
    await db.execute(sql`CREATE TABLE IF NOT EXISTS \`transfer_settlements\` (
      \`id\` int AUTO_INCREMENT PRIMARY KEY,
    ...
    await db.execute(sql`CREATE TABLE IF NOT EXISTS \`sync_failures\` (
      \`id\` int AUTO_INCREMENT PRIMARY KEY,
      \`settlementId\` int NOT NULL,
      \`failReason\` text,
